...
void PlainCodec::Encode(const Column &col, std::string *out) const {
  switch (col.type()) {
    case Type::Int32:  EncodeFixed<int32_t>(col, out); break;
    case Type::Int64:  EncodeFixed<int64_t>(col, out); break;
    case Type::Float:  EncodeFixed<float>(col, out); break;
    case Type::Double: EncodeFixed<double>(col, out); break;
    case Type::Bool:   EncodeFixed<bool>(col, out); break;
    case Type::String:
    case Type::Blob:   EncodeVar(col, out); break;
    default:           throw std::invalid_argument("PlainCodec::Encode: invalid column type");
  }
}

void PlainCodec::Decode(const Slice &bytes, size_t value_count, Column *out) const {
  switch (out->type()) {
    case Type::Int32:  DecodeFixed<int32_t>(bytes, value_count, out); break;
    case Type::Int64:  DecodeFixed<int64_t>(bytes, value_count, out); break;
    case Type::Float:  DecodeFixed<float>(bytes, value_count, out); break;
    case Type::Double: DecodeFixed<double>(bytes, value_count, out); break;
    case Type::Bool:   DecodeFixed<bool>(bytes, value_count, out); break;
    case Type::String:
    case Type::Blob:   DecodeVar(bytes, value_count, out); break;
    default:           throw std::invalid_argument("PlainCodec::Decode: invalid column type");
  }
}
