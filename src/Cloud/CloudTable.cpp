...
std::optional<TableSnapshot> CloudTable::LoadSnapshot() const {
  TableSnapshotLoader loader(descriptor_, metadata_, compacted_data_manifest_store_);
  return loader.Load();
}
...
std::unique_ptr<CloudTableCompactor> CloudTable::NewCompactor(std::shared_ptr<IObjectKeyGenerator> keys,
                                                              size_t max_publish_attempts) const {
  return std::make_unique<CloudTableCompactor>(descriptor_, files_, metadata_, compacted_data_format_, wal_format_,
